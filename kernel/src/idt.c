...
extern intr_handler interrupt_handler_table[IDT_DESC_CNT]; //引用interrupt.s中的中断处理函数入口数组
                                                           //注意，这是一个指针数组
void make_idt_desc(struct gate_desc* p_desc, char DPL, intr_handler function) {
...
void idt_desc_init(void) {
    for (int i = 0; i < IDT_DESC_CNT; i++)
    {
        ...
        make_idt_desc(&idt[i],  IDT_DESC_DPL0, interrupt_handler_table[i]);
    }
    put_str("idt is done\n",BG_BLACK+FT_YELLOW);
}
...
void idt_init() {
    ...
    pic_init();		       //初始化8259A
    load_xdt(&idt_ptr,IDT_DESC_CNT*8-1,idt);
    /* 加载idt */
